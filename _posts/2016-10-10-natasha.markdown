...
When I joined [Magenta](magenta) this summer, the team was hard at work on developing better ways to train Recurrent Neural Networks (RNNs) to generate monophonic sequences of notes. As you may remember from [previous posts](elliot post)
...
\end{align}$$
where $$c$$ is a constant controlling the emphasis placed on the music theory reward. Given the DQN loss function in Eq. $$\ref{eq:qloss}$$ and modified reward function in Eq. $$\ref{eq:reward}$$, the new loss function and learned policy for our model are,
$$\begin{align}
\label{eq:melody_dqn_loss}
&L(\theta)= \mathbb{E}_\beta[(\log p(a|s) + \frac{1}{c}r_{MT}(a,s) + \gamma \max_{a'}Q(s',a';\theta^-) - Q(s,a;\theta))^2]\\
\label{eq:melody_dqn_policy}
